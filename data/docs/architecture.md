...
3. Seed 資料（跑 `check_students_graduation` 前必要）：`python -m scripts.seed`
...
### `scripts/seed.py`
把課程、畢業規則、[Demo 測試資料](#demo-測試資料)塞進 DB。可重跑（idempotent）。
...
### `scripts/check_students_graduation.py`
對一個或多個學生跑畢業檢核。
透過這個程式，即可從資料庫調出相關規定並計算該學生是否有過，而不需啟動後端 server 或有前端網頁才可以。結果以文字或 JSON 表示。

| 指令 | 動作 |
|------|------|
| `python -m scripts.check_students_graduation 1` | 單一學生（文字摘要） |
| `python -m scripts.check_students_graduation 1 2 3` | 多個學生 |
| `python -m scripts.check_students_graduation --all` | 全部學生 |
| `python -m scripts.check_students_graduation 1 --json` | 單一學生（JSON） |
| `python -m scripts.check_students_graduation --all --json` | 全部學生 + JSON |

### `scripts/dump_erd.py`
根據目前的 SQLAlchemy models 產生 ER Diagram。
...
以下檔案是「被其他工具呼叫」的，不會也不應該直接 `python xxx.py`。列出來僅供參考。
...
| `python -m scripts.check_students_graduation 1` | `Credits: 134.0 / 128  [OK]` + `Verdict: GRADUATE` |
| `python -m scripts.check_students_graduation 2` | `Credits: 37.0 / 128  [SHORT]` + `Verdict: NOT YET` |
...
- **`check_students_graduation` 回 "No students found"** → 忘了跑 `python -m scripts.seed --only demo`
...
`seed_demo_students()` 會建立 2 個學生 + 對應的修課記錄，分別示範「達標」與「不達標」兩種情境，可直接餵給 `scripts/check_students_graduation.py` 測試畢業檢核邏輯。
...
跑 `python -m scripts.check_students_graduation 1` 預期看到：
...
跑 `python -m scripts.check_students_graduation 2` 預期看到：
